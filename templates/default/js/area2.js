...
function updateStations () {
    ...
    if(targetTr.data("job_info_id") != undefined){
        jstr += "job_info_id=" + targetTr.data("job_info_id") + '&';
    }
    jstr += 'provinceId='+$("[name='province']").val();
    jstr += '&provinceName='+$("[name='province'] option:selected").text();
    jstr += '&cityId='+$("[name='city']").val();
...
function updateJob(curNode)
{
    ...
    if("job_info_id" in area){
        targetTr.data("job_info_id",area.job_info_id);
    }
}
...
function removeAreaInfo(curNode){
    if(confirm("确定删除该点位信息？")){
        ...
        var hiddenEle = $(curNode).closest('tr').next().find("input");
        var str = hiddenEle.val();
        str = decodeURIComponent(str);
        if(str.indexOf('job_info_id') != -1){
            var strs = str.split('&');
            var positionId = strs[0].split('=')[1];
            var delJobIdsEle = $("#del_job_ids");
            console.log(delJobIdsEle);
            delJobIdsEle.val(delJobIdsEle.val() + positionId +',');
        }
        curTr.next().remove();
        curTr.remove();
    }
