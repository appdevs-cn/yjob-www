...
/*工作地区选择弹出层填充数据*/
function job_filldatas(fillID, data_resourcesP, data_resources, resultlist, showID, resultshowID, resulthidId, dir) {
	var jobhtm = '', result_datapool = new Array();
	$.each(data_resourcesP, function(indexp, valp) {
		 var pjob_array = valp.split(",");
		 	jobhtm += '<div class="data-row data-row-odd item-list clearfix">';
	...
	function copyitem() {
		...
		$("#arss").html(result_datapool.length);
		if (result_datapool.length > 0) {
			$(".cla").show();
			$(".cla").click(function() {
				result_datapool.splice(0,result_datapool.length);
	...
	function copy_selected() {
		...
		selectedcn_array.length > 0 ? selectedhtm = selectedcn_array.join("+"): selectedhtm = "选择工作地区";
		$(resultshowID).html(selectedhtm);
		$(resulthidId).val(selectedid_array.join("_"));
	}
	// 点击确定跳转
	$("#jb-selector-saves").click(function() {
		copy_selected();
		search_location(dir);	
	});
}
...
function allaround(dir) {
	...
	$('.show-more').on('click', function(event) {
		if ($(this).hasClass('sq')) {
			$(this).parent().find('.fl-content-li:gt(10)').hide();
			$(this).html('展开').removeClass('sq');
		} else {
			$(this).parent().find('.fl-content-li:gt(10)').show();
...
function search_location(dir) {
	...
	var qgdistrict=$("input[name=qgdistrict]").val();
	...
	var page=1;
	$.get(dir+"plus/ajax_search_location.php", {"qgdistrict":qgdistrict,"act":listype,"key":key,"jobcategory":jobcategory,"trade":trade,"citycategory":citycategory,"wage":wage,"jobtag":jobtag,"education":education,"experience":experience,"settr":settr,"nature":nature,"scale":scale,"sort":sort,"page":page},
...
$(function(){
	...
	if (subsiteId) {
		if(QS_city[subsiteId]){
			...
			getDistrict(subsiteArr, subsiteId, 1, "districtTwo");
		}
	} else {
		getDistrict(QS_city_parent, subsiteId, 0, "districtOne");
	};
	$("#subsiteBox").html(subsiteHtm);
	if (ishowMoreOne) {
		$('.districtOne .fl-content-li:gt(12)').hide();
	};
	if (ishowMoreTwo) {
		...
		if (arr) {
			$.each(arr, function(index, val) {
				...
				if (districtIdArr[position] == val.split(',')[0]) {
					subsiteHtm += ' select"';
					if (index > 12) {
						position == 1 ? ishowMoreTwo = false : ishowMoreOne = false;
						showMore = false;
					};
				};
				if (position == 1) {
					subsiteHtm += '" type="citycategory" code="' + parentid + '.' + val.split(',')[0] + '">' + val.split(',')[1] + '</div>';
				} else {
					subsiteHtm += '" type="citycategory" code="' + val.split(',')[0] + '.0">' + val.split(',')[1] + '</div>';
				};
			});
		};
		if (showMore) {
			if (arr.length > 13) {
				subsiteHtm += '<a href="javascript:;" class="show-more">展开</a>';
			};
		} else {
			subsiteHtm += '<a href="javascript:;" class="show-more sq">收起</a>';
		};
		subsiteHtm += '</div></div>';
		if (position == 0) {
			if (QS_city[districtIdArr[0]]) {
				var subsiteArrs = QS_city[districtIdArr[0]].split('|');
				getDistrict(subsiteArrs,districtIdArr[0], 1, "districtTwo");
			};
		};
	}
});