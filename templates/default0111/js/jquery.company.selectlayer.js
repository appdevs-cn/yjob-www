...
function job_filldatas(fillID, data_resourcesP, data_resources, resultlist, showID, resultshowID, resulthidId, dir) {
	...
	$(fillID + " a.cat").unbind().on("click",function() {
		...
		s.unbind('click').bind('click', function() {
			...
			$("#JobRequInfoTemplate a").unbind().die().live('click', function() {
				var aid = $(this).attr("data");
				$.get("company_jobs.php?act=get_content_by_qgdistrict_cat&id="+aid, function(data) {
					if (data == "-1") {
						editor.html('');
						editor.sync();
					} else {
			...
			$("#jobTexts").html(sdataArray[1]);
			$("#category_cns").val(sdataArray[1]);
			$("#topclasss").val(checkIDArray[0]);
			$("#categorys").val(checkIDArray[1]);
			$("#subclasss").val(checkIDArray[2]);
			$("#jobTexts").parent().find('.input_text_200_bg').removeClass('error');
			$("#jobTexts").parent().next().find('.error').hide();
			$('.aui_outer').hide();
			$(".ucc-default").removeClass('aui_is_show');
		});
	});
...
function trade_filldata(fillID, data_resources, showID, resultlist, resultshowID, resultID, dir) {
	...
	$(fillID+" label").unbind().live("click",function(event) {
		...
		$("#trade").parent().find('.input_text_200_bg').removeClass('error');
		$("#trade").parent().next().find('.error').hide();
		$('.aui_outer').hide();
...
/*职位亮点选择弹出层填充数据*/
function tag_filldatas(fillID, data_resources, showID, resultlist, resultshowID, resultID, dir) {
	var tradhtm = '';
	var sourse_length = parseInt(data_resources.length);
	var rows = 0;
	var subscriptnum = 0;
	...
	function copyitem() {
		...
		$("#arstrades").html($(fillID+" :checkbox[checked]").length);
		if ($(fillID+" :checkbox[checked]").length > 0) {
			$(".cla").show();
			$(".cla").click(function() {
				$(fillID+" :checkbox").unbind().each(function(index) {
	...
	function copy_selected() {
		...
		$(".showchecktags").html(tagotphtm);
		$(resultID).val(selectedid_array.join(","));
		$("#tag_cns").val(selectedcn_array.join(","));
	}
	// 分割data 返回数组
	function splitdata(arr) {
		if(arr) {
	...
	// 点击确定
	$("#tag-selector-saves").click(function() {
		copy_selected();
		$('.aui_outer').hide();
		$("#maskLayerLoad").remove();
		$(".ucc-default").removeClass('aui_is_show');
