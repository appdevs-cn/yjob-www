...
if($act =='do_login')
{
	...
	{
		//用户名为 前缀+邮箱地址 的时候也将其判断为邮箱，因此判断其是否以前缀开头的，若 是用户名 否则是邮箱
		if(strpos($username, $_CFG['email_reg_prefix'])===0)
		{
			$account_type=1;
		}
		else
...
elseif($act == "reg_send_sms11")
{
	...
	if(!empty($sms_type))
	{
		...
		if($db->getone("select * from qs_members where mobile='{$mobile}'")){
			exit("手机号已经注册！");
		}
		$rand=mt_rand(100000, 999999);	
		switch ($sms_type) {
