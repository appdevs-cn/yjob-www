			...
			else
			{
					...
					if ($_SESSION['utype']=="2")
					{
						...
						if ($rule['per_verifymobile']['value']>0)
						{
							...
							write_memberslog($_SESSION['uid'],2,9001,$_SESSION['username']," 手机通过验证，{$_CFG['points_byname']}({$operator}{$rule['per_verifymobile']['value']})，(剩余:{$user_points})",2,1015,"手机认证通过","{$operator}{$rule['per_verifymobile']['value']}","{$user_points}");
							}
						}
					}
