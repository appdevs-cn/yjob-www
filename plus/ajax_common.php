...
elseif ($act=="company_down_resume")
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		if($_CFG['closetime']==1){
			$html[$row['bid']]="<li><a href=".url_rewrite('QS_companyshow',array('id'=>$row['bid']))." target=\"_blank\">".$row['companyname']." </a> 下载了 <a href=".url_rewrite('QS_resumeshow',array('id'=>$row['cid']),1,$row['subsite_id'])." target=\"_blank\">".$row['fullname']." </a>的个人简历</li>";
		}else{
			$html[$row['bid']]="<li><a href=".url_rewrite('QS_companyshow',array('id'=>$row['bid']))." target=\"_blank\">".$row['companyname']." </a> 下载了 <a href=".url_rewrite('QS_resumeshow',array('id'=>$row['cid']),1,$row['subsite_id'])." target=\"_blank\">".$row['fullname']." </a>的个人简历<span>{$row['time']}</span></li>";
		}
		}
	}
	exit(implode("",$html));
