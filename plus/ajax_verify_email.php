			...
			else
			{
					...
					if ($_SESSION['utype']=="2")
					{
						...
						if ($rule['per_verifyemail']['value']>0)
						{
							...
							write_memberslog($_SESSION['uid'],2,9001,$_SESSION['username']," 邮箱通过验证，{$_CFG['points_byname']}({$operator}{$rule['per_verifyemail']['value']})，(剩余:{$user_points})",2,1015,"邮箱认证通过","{$operator}{$rule['per_verifyemail']['value']}","{$user_points}");
							}
						}
					}
