...
$(document).ready(function() {
 		...
 		layer.open({
			...
			yes: function(){
                var optionOnArr = $(".formDivlayer .on"),
                    tradeIdArr = new Array(),
                    tradeValArr = new Array();
                $.each(optionOnArr, function(index, val) {
                    tradeIdArr[index] = $(this).data('val');
                    tradeValArr[index] = $(this).data('content');
                });
                $("#trade").val(tradeIdArr.join(","));
                $("#trade_cn").val(tradeValArr.join(","));
                $("#tradeTxt").text(tradeValArr.join(","));
                layer.closeAll();
            }, no: function(){
                layer.closeAll();
            },
	...
	/* 期望职位 */
 	$("#jobTxt").on('click', function(event) {
 		layer.open({
 			title: [
		        '请选择期望职位'
		    ],
 			content: getOptionsContent(QS_jobs_ds, ''),
		    btn: ['确认','取消'],
		    shadeClose: true,
			yes: function(){
                var optionOnArr = $(".formDivlayer .on"),
                    jobsIdArr = new Array(),
                    jobsValArr = new Array();
                $.each(optionOnArr, function(index, val) {
                    jobsIdArr[index] = '1.1065.'+$(this).data('val');
                    jobsValArr[index] = $(this).data('content');
                });
                $("#intention_jobs_id").val(jobsIdArr.join(","));
                $("#intention_jobs").val(jobsValArr.join(","));
                $("#jobTxt").text(jobsValArr.join(","));
                layer.closeAll();
            }, no: function(){
                layer.closeAll();
            },
 	...
 	/* $("#jobTxt").on('click', function(event) {
 		layer.open({
 			...
 			content: getOptionsContent(QS_jobs, ''),
		    btn: ['确认','取消'],
		    shadeClose: true,
			yes: function(){
                var optionOnArr = $(".formDivlayer .on"),
                    ...
                    jobValArr = new Array();
                $.each(optionOnArr, function(index, val) {
                    jobIdArr[index] = $(this).data('val');
                    jobValArr[index] = $(this).data('content');
                });
                $("#intention_jobs_id").val(jobIdArr.join(","));
                $("#intention_jobs").val(jobValArr.join(","));
                $("#jobTxt").text(jobValArr.join(","));
                layer.closeAll();
            }, no: function(){
                layer.closeAll();
            },
