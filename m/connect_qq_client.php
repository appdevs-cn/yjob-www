...
if($act == 'QQlogin')
{
$url="https://graph.qq.com/oauth2.0/authorize?response_type=token&client_id={$_CFG['qq_appid']}&redirect_uri=http://www.yjob.net/m/connect_qq_client.php".urlencode('?act=login_check');
header("Location:{$url}");	
}
...
elseif ($act=='reg_save')
{
	...
	$val['mobile']=!empty($_POST['mobile'])?trim($_POST['mobile']):exit("输入手机号");
	$val['password']=!empty($_POST['password'])?trim($_POST['password']):exit("输入密码");
	if($val['password']!=trim($_POST['rpassword'])){
		exit("密码不一致");
	}
	$verifycodekkk=trim($_POST['verifycode']);
	$verifycode=substr(md5($verifycodekkk), 8,16);
	if (empty($verifycodekkk) || empty($_SESSION['mobile_rand']) || $verifycode<>$_SESSION['mobile_rand'])
	{
		exit("验证码错误！");
	}
	require_once(QISHI_ROOT_PATH.'include/mysql.class.php');
	...
	$userid=user_register(2,$val['password'],$val['member_type'],$val['email'],$val['mobile']);
	if ($userid)
	{
