...
elseif($act=="jobs_add_save")
{
	...
	$setsqlarr['work_start']=intval(convert_datefm($_POST['work_start'],2));
	$setsqlarr['work_end']=intval(convert_datefm($_POST['work_end'],2));
	$setsqlarr['sex']=3;
	$setsqlarr['sex_cn']="不限";
		...
		else
		{
			...
			if($setmeal['refresh_jobs_time']!=0&&($jobs_num>$surplus_time))
			{
				...
				if($_CFG['setmeal_to_points']=='1')
				{
					...
					if($_CFG['com_pointsmode_refresh_time']!=0&&($refresh_time['count(*)']>=$_CFG['com_pointsmode_refresh_time']))
					{
						exit("每天用积分刷新最多只能刷新".$_CFG['com_pointsmode_refresh_time']."次,您今天已超过最大刷新次数限制！");
					}
					elseif($duringtime<=$space)
					{
						exit($_CFG['com_pointsmode_refresh_space']."分钟内不能重复刷新职位！");
