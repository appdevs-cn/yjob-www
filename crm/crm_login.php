...
if($act == 'logout')
{
	...
	unset($_SESSION['crm_admin_name']);
	unset($_SESSION['crm_admin_purview']);
	setcookie('Qishi[crm_admin_id]',"",time() - 3600,$QS_cookiepath, $QS_cookiedomain);
	setcookie('Qishi[crm_admin_name]',"",time() - 3600,$QS_cookiepath, $QS_cookiedomain);
	setcookie('Qishi[crm_admin_pwd]',"",time() - 3600,$QS_cookiepath, $QS_cookiedomain);
...
elseif($act == 'login')
{
	...
	if(isset($_SESSION['crm_admin_id'],$_SESSION['crm_admin_name'],$_SESSION['crm_admin_purview']))
	{
	header("Location: crm_index.php"); 
	}
	...
	$smarty->display('sys/crm_login.htm');
}
elseif($act == 'do_login')
{
	header("Expires: Mon, 26 Jul 1997 05:00:00 GMT");
 	...
 	elseif(check_admin($admin_name,$admin_pwd))
	{
 		...
 		if($remember == 1)
		{
			$admininfo=get_admin_one($admin_name);
 			setcookie('Qishi[crm_admin_id]', $_SESSION['crm_admin_id'], time()+86400, $QS_cookiepath, $QS_cookiedomain);
 			setcookie('Qishi[crm_admin_name]', $admin_name, time()+86400, $QS_cookiepath, $QS_cookiedomain);
			setcookie('Qishi[crm_admin_pwd]', md5($admin_name.$admininfo['pwd'].$admininfo['pwd_hash'].$QS_pwdhash), time()+86400, $QS_cookiepath, $QS_cookiedomain);
 		}
 	}
	else
	{
