...
{
	global $db;
	$sql = "select * from ".table('crm_category_group');
	return $db->getall($sql);
}
function get_category_group_one($alias)
{
	global $db;
	$sql = "select * from ".table('crm_category_group')." WHERE g_alias='".$alias."'";
...
function del_group($alias)
{
	...
	foreach($alias as $a)
	{
			if (!$db->query("Delete from ".table('crm_category_group')." WHERE g_alias ='".trim($a)."' AND g_sys<>1")) return false;
			$return=$return+$db->affected_rows();
			if (!$db->query("Delete from ".table('crm_category')." WHERE c_alias ='".trim($a)."' ")) return false;
			$return=$return+$db->affected_rows();
	}
	return $return;
}
...
{
	global $db;
	$sql = "select * from ".table('crm_category')." WHERE c_alias='".$alias."'  ORDER BY c_order DESC,c_id ASC";
	return $db->getall($sql);
}
function get_category_one($id)
{
	global $db;
	$sql = "select * from ".table('crm_category')." WHERE c_id=".intval($id)." LIMIT 1";
	return $db->getone($sql);
}
function del_category($id)
{
	...
	{
		if (!$db->query("Delete from ".table('crm_category')." WHERE c_id IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
	}
	return $return;
}
