...
{
if (!file_exists($dir)) crmmsg("上传图片失败：上传目录 ".$dir." 不存在!",0);
if (!is_writable($dir)) 
{
crmmsg("上传图片失败：上传目录 ".$dir." 无法写入!",0);
exit(); 
}
...
{ 
crmmsg('上传图片失败：你选择的文件无法上传',0);
exit(); 
} 
if ($max_size>0 && $upfile['size']/1024>$max_size) 
{ 
crmmsg("上传图片失败：文件大小不能超过  ".$max_size."KB",0);
exit(); 
} 
$ext_name = strtolower(str_replace(".", "", strrchr($upfilename, "."))); 
if (!($type==='') && strpos($type, $ext_name)===false) 
{ 
crmmsg("上传图片失败：只允许上传 ".$type." 的文件！",0);
exit(); 
}
