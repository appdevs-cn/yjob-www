...
elseif($act == 'editsave')
{
	...
	$link[1]['href'] = "?act=edit&id=".$id;
 	!updatetable(table('crm_business_dairy'),$setsqlarr," id=".$id."")?crmmsg("修改失败！",0):crmmsg("修改成功！",2,$link);
}
elseif($act == 'add')
{
	get_token();
...
{	
	...
	$link[1]['text'] = "继续添加";
	$link[1]['href'] = '?act=add';
	!inserttable(table('crm_business_dairy'),$setsqlarr)?crmmsg("添加失败！",0):crmmsg("添加成功！",2,$link);
}
