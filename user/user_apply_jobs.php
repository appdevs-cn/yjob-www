...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tableall" id="app">
    ...
    <tr>
		<td width="120" align="right">请确认报名：</td>
		<td style="display:none;">
			<select  name="resumeid"  id="resumeid">
				...
				{
				?>
				<option value="<?php echo $resume['id']?>" selected="selected"><?php echo $resume['nickname']?> </option>
				<?php
				}
				?>
			</select>
    ...
    <tr>
		<td></td>
		<td>
			<input type="button" name="Submit"  id="ajax_app" class="but130lan" value="确定" />
		</td>
    </tr>
</table>
...
elseif ($act=="app_save")
{
	...
	if(empty($resume_basic['sex_cn']) || empty($resume_basic['fullname'])){
		echo '<script type="text/javascript">
			alert("请先完善个人简历信息！");
			window.location.href="../user/personal/personal_resume.php?act=edit_resume&pid='.$resumeid.'&make=1";
		</script>';
	}else{
		$i=0;
