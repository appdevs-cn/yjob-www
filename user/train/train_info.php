...
elseif ($act=='train_profile_save')
{
	...
	if ($train_profile)
	{
			...
			if ($db->updatetable(table('train_profile'), $setsqlarr," uid='{$uid}'"))
			{
				...
				if (!$db->updatetable(table('course'),$coursearr," uid=".$setsqlarr['uid']."")) showmsg('修改培训机构名称出错！',0);
				if (!$db->updatetable(table('train_teachers'),$coursearr," uid=".$setsqlarr['uid']."")) showmsg('修改培训机构名称出错！',0);
...
elseif ($act=='train_auth_save')
{
	...
	!$db->updatetable(table('train_profile'),$setsqlarr,$wheresql)?showmsg('保存失败！',1):showmsg('保存成功，请耐心等待管理员审核！',2);
	}
	else
	{
	showmsg('保存失败！',1);
...
elseif ($act=='train_news_add_save')
{
	...
	!$db->inserttable(table('train_news'),$setsqlarr)?showmsg("添加失败！",0):showmsg("添加成功！",2,$link);
}
...
elseif ($act=='train_news_edit_save')
{
	...
	$uid=intval($_SESSION['uid']);
	$id=intval($_POST['id']);
	!$db->updatetable(table('train_news'),$setsqlarr," uid='{$uid}' AND id='{$id}' ")?showmsg("修改失败！",0):showmsg("修改成功！",2,$link);
}
