...
<div class="interview-dialog dialog-block">
	...
	<div class="dialog-item clearfix">
		...
		<div class="d-content f-left">
			<div class="f-left"><label><input <?php if(intval($user['sms_num']) == 0) echo "disabled";?> type="checkbox" name="sms_notice" id="sms_notice" class="checkbox" />是</label></div>
			<?php if($_CFG['company_sms']==1) {?><div class="short-text-tip f-left" >您的可用短信条数为 <span style="color:#ff9900;"><?php echo $user['sms_num'];?></span> 条 <a target="_blank" href="../user/company/company_sms.php?act=sms_order" class="underline">立即购买</a></div><?php } ?>
		</div>
	</div>
...
elseif ($act=="invited_save")
{
	...
	*/
	...
	if ($mailconfig['set_invite']=="1" && $resume_user['email_audit']=="1")
	{
	dfopen($_CFG['site_domain'].$_CFG['site_dir']."plus/asyn_mail.php?uid={$_SESSION['uid']}&key=".asyn_userkey($_SESSION['uid'])."&act=set_invite&companyname={$jobs['companyname']}&email={$resume['email']}");
	}
	//站内信
	if($pms_notice=='1'){
