...
elseif($act =="reg_step3")
{
	...
	if($reg_type==1)
	{
		...
		$uid=$db->getone("select * from ".table('members')." where username='{$mobile}'");
		$sete['telephone']=$mobile;
		$setet['phone']=$mobile;
		$sete['uid']=$uid['uid'];
		$setet['uid']=$uid['uid'];
		//$reek=$db->inserttable(table('resume'),$sete);
	...
	if($rst>0)
	{
		...
		{
			$uid=$_SESSION['uid'];
			$cxx=$db->getone('select id from '.table('resume').' where uid='.$uid);
			$user['index_url']=$_CFG['site_domain'].$_CFG['site_dir']."user/personal/personal_resume.php?act=edit_resume&pid='{$cxx['id']}'&make=1";
		}
		elseif($user['utype']=='3')
		{
