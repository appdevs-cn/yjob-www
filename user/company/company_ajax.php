...
elseif($act=='order_detail')
{
	...
	exit($contents);
} elseif ($act == 'get_cat_qgdistrict') {
    $pid=intval($_GET['pid']);
    $sql = "select * from ".table('category_qgdistrict')." where parentid='{$pid}'  order BY category_order desc,id asc";
    $result = $db->query($sql);
    while($row = $db->fetch_array($result))
    {
        $cat[]=$row['id'].",".$row['categoryname'].",".$row['category_order'].",".$row['lng'].",".$row['lat'];
    }
    if (!empty($cat))
    {
        exit(implode('|',$cat));
...
elseif($act == "jobs_refresh_ajax")
{
	...
	$jobsid=$_GET['jobsid']?intval($_GET['jobsid']):exit('职位ID丢失！');
        $jobs_info_tmp = https_request_api("job/info/".$jobsid);
        if($jobs_info_tmp['codes'] == 'N') {
            exit('<div class="del-dialog"><div class="tip-block"><span class="del-tips-text">职位信息丢失！</span></div></div>');
        }
