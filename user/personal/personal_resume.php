...
elseif ($act=='make1_save')
{
	...
	//昵称
	$setsqlarr['nickname']=trim($_POST['nickname'])?trim($_POST['nickname']):showmsg('请填写昵称！',1);
	//上岗时间
	$setsqlarr['job_time']=trim($_POST['job_time'])?trim($_POST['job_time']):showmsg('请填写上岗时间！',1);
	//上岗次数
	$setsqlarr['job_number']=$_POST['job_number'];
	//取消报名
	$setsqlarr['cancel_sign']=$_POST['cancel_sign'];
	//放鸽子数
	$setsqlarr['fly_number']=$_POST['fly_number'];
	//简历照片
	$setsqlarr['photo_img']=$_POST['photo'];
...
elseif($act == 'ajax_save_basic_info')
{
	...
	$setsqlarr['nickname']=utf8_to_gbk(trim($_POST['nickname']));
	...
	//var_dump(utf8_to_gbk(trim($_POST['schoolname'])));exit();
	...
	//$setsqlarr['email']=trim($_POST['email'])?utf8_to_gbk(trim($_POST['email'])):'';
	/* if($user['email_audit']!="1")
	{
		$members['email']=$setsqlarr['email'];
		$members_info['email']=$setsqlarr['email'];
	...
	$householdaddress=utf8_to_gbk(trim($_POST['householdaddress']));
	$realname=$setsqlarr['fullname'];
	$db->updatetable(table('resume'),$setsqlarr, array("id"=>intval($_REQUEST['pid']),"uid"=>$_SESSION['uid']));
	$db->query("update ".table('members_info')." set height='{$height}',householdaddress='{$householdaddress}',realname='{$realname}',sex='{$setsqlarr['sex']}',sex_cn='{$setsqlarr['sex_cn']}',birthday='{$setsqlarr['birthdate']}',marriage='{$setsqlarr['marriage']}',marriage_cn='{$setsqlarr['marriage_cn']}' where uid='{$_SESSION['uid']}'");//新加
	...
	//$nickname = utf8_to_gbk(trim($_POST['nickname']));
...
elseif ($act=='ajax_save_basic')
{
	...
	$setsqlarr['district_cn']=!utf8_to_gbk(trim($_POST['district_cn']))?utf8_to_gbk(trim($_POST['subsite_name'])):(utf8_to_gbk(trim($_POST['district_cn'])));
	...
	if(empty($cxsearch)){
		...
		$db->query("update ".table('resume_search_key')." set subsite_id=".$setsqlarr['subsite_id']." where id=".$_REQUEST['pid']);
		$db->query("update ".table('resume_search_rtime')." set subsite_id=".$setsqlarr['subsite_id']." where id=".$_REQUEST['pid']);
	}
	check_resume($_SESSION['uid'],intval($_REQUEST['pid']));
	if($_CFG['audit_edit_resume']!="-1"){
		set_resume_entrust(intval($_REQUEST['pid']));
...
{
	$uid=intval($_SESSION['uid']);
	$pid=$_POST['pid']?intval($_POST['pid']):'';
	...
	$resume_basic= get_resume_basic($uid,$pid);
		...
		if($auto_apply==1)
		{
			...
			if(empty($auto_apply_row))
			{
				...
				$setarr_apply['entrust_end']=$time1=$time+3600*24*3;;
				$setarr_apply['isshield']=0;
				$setarr_apply['resume_addtime']=$atime=$resume_basic['addtime'];
				$db->query("update ".table("resume_entrust")." set uid='{$uid}',fullname='{$resume_basic['fullname']}',entrust='1',entrust_start='{$time}',entrust_end='{$time1}',isshield='0',resume_addtime='{$atime}' where id='{$pid}'");
...
elseif ($act=='edit_resume')
{
	...
	{ */
		$title="修改简历";
	/* } */
	$worknum=$db->get_total("select COUNT(*) AS num from  ".table("company_label_resume")." where resume_id=".$pid." and resume_state=3 ");
	$worknum1=$db->get_total("select COUNT(*) AS num from  ".table("company_label_resume")." where resume_id=".$pid." and resume_state=4 ");
	$smarty->assign('h_title',$title);
	$_SESSION['send_mobile_key']=mt_rand(100000, 999999);
	$smarty->assign('send_key',$_SESSION['send_mobile_key']);
	$resume_basic = get_resume_basic($uid,$pid);
	if(empty($resume_basic)){
		$sqll="select * from ".table('members')." where uid='{$uid}'";
		$resume_basic=$db->getone($sqll);
		$resume_basic['telephone']=$resume_basic['mobile'];
	}
	$sql2="select * from ".table('personal_jobs_apply')." where personal_uid='{$uid}' limit 2";
	$resume_work=$db->getall($sql2);
	$resume_basic['biyetime']=cut_str($resume_basic['biyetime'],5,true);
	...
	//$smarty->assign('worknum',$worknum);
	//$smarty->assign('worknum1',$worknum1);
	$smarty->assign('subsite',get_all_subsite());
