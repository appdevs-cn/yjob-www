...
function add_recharge($uid,$pay_type,$oid,$amount,$payment_name,$description,$addtime,$points='',$setmeal='',$utype='1')
{
	global $db;
	$setsqlarr['uid']=intval($uid);
	$setsqlarr['pay_type']=$pay_type;
...
function add_recharge_resume($uid,$pay_type,$oid,$amount,$payment_name,$description,$addtime,$points='',$setmeal='',$utype='1')
{
	global $db;
	$setsqlarr['uid']=intval($uid);
	$setsqlarr['pay_type']=$pay_type;
...
{
	global $db;
	$sql = "select * from ".table('order')." WHERE uid=".intval($uid)." AND  is_paid='".intval($is_paid)."' and recharge =0  ORDER BY id desc";
	return $db->getall($sql);
}
//取消订单
...
function recharge_rmb($v_oid )
{
	global $db,$timestamp,$_CFG; 
	$order=$db->getone("select * from ".table('order')." WHERE oid ='{$v_oid}' AND recharge= '1' and is_paid=2 LIMIT 1 ");
	$uid = $order['uid'];
	$sql = "UPDATE ".table('members_setmeal')." SET jobsfair_num= '1'  WHERE uid='{$uid}' LIMIT 1 ";
	return $db->query($sql);
}
...
function recharge_rmb_resume($v_oid )
{
	global $db,$timestamp,$_CFG; 
	$order=$db->getone("select * from ".table('order')." WHERE oid ='{$v_oid}' AND recharge= '2' and is_paid=2 LIMIT 1 ");
	$uid = $order['uid'];
	
	$sql = "UPDATE ".table('members_setmeal')." SET download_resume_ordinary= '1'  WHERE uid='{$uid}' LIMIT 1 ";
	return $db->query($sql);
}
...
function get_apply_jobs($offset,$perpage,$get_sql= '')
{
	global $db;
	$limit=" LIMIT {$offset},{$perpage}";
	$selectstr=" a.*,r.id,r.uid as ruid,r.fullname,r.display_name,r.sex_cn,r.sex,r.education_cn,r.experience_cn,r.major_cn,r.intention_jobs,r.district_cn,r.telephone,r.wage_cn,r.trade_cn,r.nature_cn,r.birthdate,r.addtime,r.refreshtime,r.display";
	...
	while($row = $db->fetch_array($result))
	{
	    ...
	    $results=$db->getall("select * from ".table('jobs')." WHERE id='{$row[jobs_id]}' ");
		$row['work_start']=$results['work_start'];
		$time=time();
		$times=$_CFG['del_time'];
		$time1=$times*24*60*60;
		if($row['work_start']-$time>$time1){
			$row["work_starts"]=1;
			}else{
				$row["work_starts"]=2;
				}
		...
		/*
			获取简历标记
		*/
		$row_state=get_resume_state($_SESSION['uid'],$row['resume_id'],$row['jobs_id']);
		$row['resume_state']=$row_state['resume_state'];
		$row['resume_state_cn']=$row_state['resume_state_cn'];
		$row_arr[] = $row;
	}
...
function get_jobfair_exhibitors($offset, $perpage, $sql= '')
{
	...
	$time=time();
	$limit=" LIMIT {$offset},{$perpage}";
	$result = $db->query("SELECT e.*,j.title,j.holddate_start,j.holddate_end,j.addtime FROM ".table('jobfair_exhibitors')." AS e ".$sql." ".$limit);
	while($row = $db->fetch_array($result))
	{
...
function get_user_seatnumber($uid)
{
	global $db;
	$uid=intval($uid);
	$sql = "select * from ".table('members_setmeal')."  WHERE uid='{$uid}' AND  effective=1 LIMIT 1";
	$member_meal=$db->getone($sql);
	$set_meal=$db->getone("select reg_times from ".table('setmeal')."  WHERE id='{$member_meal['setmeal_id']}'  LIMIT 1");
	return $set_meal['reg_times'];
}
function get_exhibitors($uid)
{
	global $db;
	$uid=intval($uid);
	$sql = "select id from ".table('jobfair_exhibitors')."  WHERE uid='{$uid}'";
	$t=$db->getall($sql);
	return count($t);
}
/*
...
function get_content_by_qgdistrict_cat($id){
	global $db;
	$content = $db->getone("select content from ".table('category_qgdistrict')." where id=".$id);
	return $content['content'];
}
function get_color()
{
...
function get_my_attention($offset,$perpage,$get_sql= '')
{
	global $db;
	$limit=" LIMIT {$offset},{$perpage}";
	$selectstr=" a.*,r.uid as ruid,r.fullname,r.display_name,r.sex_cn,r.sex,r.education_cn,r.experience_cn,r.intention_jobs,r.district_cn,r.wage_cn,r.trade_cn,r.nature_cn,r.birthdate,r.refreshtime";
	$result = $db->query("SELECT {$selectstr} FROM ".table('view_resume')." as a {$get_sql} ORDER BY a.id DESC {$limit}");
	while($row = $db->fetch_array($result))
	{
		$row['url'] = url_rewrite("QS_resumeshow",array('id'=>$row['resumeid']));
...
function del_my_attention($del_id,$uid)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_memberslog($_SESSION['uid'],$_SESSION['utype'],1605,$_SESSION['username'],"删除浏览过的简历({$sqlin})");		
		return $return;
	}
	else
	{
...
function get_adv_list(){
	global $db;
	$sql = "select * from ".table("ad_category")." where qtxs=1";
	return $db->getall($sql);
}
...
function get_view_users($offset,$perpage,$get_sql= '')
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		$row['jobs_url'] = url_rewrite("QS_jobsshow",array('id'=>$row['jobsid']));
		...
		foreach ($resumes as $key1 => $value1) {
			$value1['resume_url']=url_rewrite("QS_resumeshow",array('id'=>$value1['id']));
...
function add_sms_order($uid,$oid,$amount,$payment_name,$description,$addtime,$points,$setmeal)
{
	...
	$setsqlarr['pay_type']='3';		//1为套餐 2。为广告位 3为短信
	$setsqlarr['oid']=$oid;
	$setsqlarr['amount']=$amount; 
	$setsqlarr['payment_name']=$payment_name;
	$setsqlarr['points']=$points;
	$setsqlarr['addtime']=$addtime;
	$setsqlarr['description']=$description; 
	$setsqlarr['setmeal']=$setmeal;	//套餐ID
	
	write_memberslog($uid,1,3002,$_SESSION['username'],"添加订单，编号{$oid}，金额{$amount}元"); 
	return $db->inserttable(table('order'),$setsqlarr,true);
}
//取消订单
...
{
	global $db;
	write_memberslog($_SESSION['uid'],1,3003,$_SESSION['username'],"取消短信订单，短信订单id{$id}");
	return $db->query("Delete from ".table('order')." WHERE id='".intval($id)."' AND uid=".intval($uid)." AND is_paid=1  LIMIT 1 ");
}
//申请短信付款后开通
...
function get_resume_state($uid,$resume_id,$jobs_id)
{
	global $db;
	$uid=intval($uid);
	$resume_id=intval($resume_id);
	if($jobs_id){
		return $db->getone( "select resume_state,resume_state_cn from ".table('company_label_resume')." WHERE uid={$uid} and resume_id=$resume_id and jobs_id=$jobs_id limit 1 ");
	}else{
