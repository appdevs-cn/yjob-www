...
//注册会员
function user_register($reg_type,$password,$member_type=0,$email="",$mobile="",$uc_reg=true,$username="",$weixin_openid="",$weixin_nickname="", $icode='')
{
	...
	$insert_id=$db->inserttable(table('members'),$setsqlarr,true);
			if($member_type=="1")
			{
				...
				$setarr['uid']=$insert_id;
				$setinfo['uid']=$insert_id;
				$setinfo['phone']=$mobile;
				$setresume['telephone']=$mobile;
				$setresume['uid']=$insert_id;
				if(!$db->inserttable(table("members_points"),$setarr))  return false;
				if(!$db->inserttable(table("members_info"),$setinfo))  return false;
				if(!$db->inserttable(table("resume"),$setresume))  return false;
				$points=get_cache('points_rule');
...
//检测COOKIE
function check_cookies($name,$pwd){
 	global $db;
...
function check_cookie($uid,$name,$pwd){
 ...
 function update_user_info($uid,$record=true,$setcookie=true,$cookie_expire=NULL)
 {
	...
	setcookie('QS[pmscount]',$pmscount, $expire,$QS_cookiepath);
	return true;
 }
  function update_user_infos($uid,$record=true,$setcookie=true,$cookie_expire=NULL)
 {
 	global $timestamp, $online_ip,$db,$QS_cookiepath,$QS_cookiedomain,$_CFG;//3.4升级修改 引入变量$_CFG
	$user = get_user_inid($uid);
	if (empty($user))
...
function edit_username($arr,$check=true)
{
	...
	if ($db->query( "UPDATE ".table('members')." SET username = '{$arr['newusername']}'  WHERE uid='".$arr['uid']."'"))
	{
		...
		setcookie('QS[username]',$arr['newusername'],$expire,$QS_cookiepath);
