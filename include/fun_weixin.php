 ...
 * $keyword3 				金额
*/
function set_apply_heshi($uid,$jobs_name,$jobs_id,$fullname,$jobs_contont)
{
	
	global $db,$_CFG;
	$weixinconfig=get_cache('weixin_config');
	$user['weixin_openid']=$uid;
	if(intval($_CFG['weixin_apiopen'])==1)
	{
		$jobss_url=$_CFG['wap_domain']."/jobs-show.php?id=".$jobs_id;
			$template = array(
				'touser' => $user['weixin_openid'],
				'template_id' => "0-ehktbeHlxvzLdo8PaXQHUzJHR8eywcVNAWsEqCZ2E",
				'url' => $jobss_url,
				'topcolor' => "#17A8EB",
				'data' => array(
					'first' => array('value' => urlencode(gbk_to_utf8("您申请".$jobs_name."职位已经通过审核")),
									'color' => "#17A8EB",
						),
					'keyword1' => array('value' => urlencode(gbk_to_utf8($fullname)),
									'color' => "#17A8EB",
						),
					'keyword2' => array('value' => urlencode(gbk_to_utf8($jobs_name)),
									'color' => "#17A8EB",
						),
					'keyword3' => array('value' => urlencode(gbk_to_utf8(date('Y-m-d',time()))),
									'color' => "#17A8EB",
						),
					'remark' => array('value' => urlencode(gbk_to_utf8("\\n".$jobs_contont)),
									'color' => "#17A8EB",
						)
					)
				);
 ...
 * $keyword3 				金额
*/
function set_apply_buheshi($uid,$fullname,$jobs_name,$jobs_id)
{
	global $db,$_CFG;
	$weixinconfig=get_cache('weixin_config');
	$user['weixin_openid']=$uid;
	if(intval($_CFG['weixin_apiopen'])==1)
	{
		...
		$jobss_url=$_CFG['wap_domain']."/jobs-show.php?id=".$jobs_id;
			$template = array(
				'touser' => $user['weixin_openid'],
				'template_id' => "pBnuyaYVste4HKKFUe2nP1C4B6ydfudnKNQle8z9bJ8",
				'url' =>$jobss_url,
				'topcolor' => "#17A8EB",
				'data' => array(
					'first' => array('value' => urlencode(gbk_to_utf8("抱歉，您的申请已经被拒绝")),
									'color' => "#17A8EB",
						),
					'keyword1' => array('value' => urlencode(gbk_to_utf8($fullname)),
									'color' => "#17A8EB",
					...
					'remark' => array('value' => urlencode(gbk_to_utf8("查看工作详情")),
									'color' => "#17A8EB",
						)
					)
				);
 ...
 * $keyword3 				金额
*/
function set_apply_fanggezi($uid,$keywords,$jobs_id)
{
	global $db,$_CFG;
	$weixinconfig=get_cache('weixin_config');
	$user['weixin_openid']=$uid;
	if(intval($_CFG['weixin_apiopen'])==1)
	{
		...
		$jobss_url=$_CFG['wap_domain']."/jobs-show.php?id=".$jobs_id;
			$template = array(
				'touser' => $user['weixin_openid'],
				'template_id' => "LTER8TYCZMqP8NWW82lsHX6MGU0YmZpS9AYYu0uck_g",
				'url' => $jobss_url,
				'topcolor' => "#17A8EB",
				'data' => array(
					'first' => array('value' => urlencode(gbk_to_utf8("您的状态已经被标记为放鸽子")),
									'color' => "#17A8EB",
						),
					'keyword1' => array('value' => urlencode(gbk_to_utf8("放鸽子")),
									'color' => "#17A8EB",
						),
					'keyword2' => array('value' => urlencode(gbk_to_utf8(date('Y年m月d日 H:i',time()))),
									'color' => "#17A8EB",
						),
					'remark' => array('value' => urlencode(gbk_to_utf8("\\n".$keywords)),
									'color' => "#17A8EB",
 ...
 * $keyword3 				金额
*/
function set_apply_quxiao($uid,$jobs_name,$fullname,$phone,$jobs_id)
{
	...
	if(intval($_CFG['weixin_apiopen'])==1)
	{
		$jobss_url=$_CFG['wap_domain']."/jobs-show.php?id=".$jobs_id;
			$template = array(
				'touser' => $user['weixin_openid'],
				'template_id' => "RjofPXhr9i9yzSUKSR3rkdzUcM7wvGe610hCDjH43K4",
				'url' => $jobss_url,
				'topcolor' => "#7B68EE",
				'data' => array(
					'first' => array('value' => urlencode(gbk_to_utf8("您好！您申请的工作已经取消了！")),
									'color' => "#743A3A",
						),
					'keyword1' => array('value' => urlencode(gbk_to_utf8($jobs_name)),
									'color' => "#743A3A",
						),
					'keyword2' => array('value' => urlencode(gbk_to_utf8($fullname)),
									'color' => "#743A3A",
						),
					'keyword3' => array('value' => urlencode(gbk_to_utf8($phone)),
									'color' => "#743A3A",
						),
					'keyword4' => array('value' => urlencode(gbk_to_utf8(date('Y-m-d H:i:s',time()))),
									'color' => "#743A3A",
						),
					'remark' => array('value' => urlencode(gbk_to_utf8("点击查看兼职工作详情。")),
									'color' => "#743A3A",
						)
					)
				);
