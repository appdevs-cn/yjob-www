...
function get_all_district_marked_subsite(){
	global $db;
...
function send_sms($mobile,$content)
{
	...
	{
		//var_dump("http://www.74cms.com/SMSsend.php?sms_name={$sms['notice_sms_name']}&sms_key={$sms['notice_sms_key']}&mobile={$mobile}&content={$content}");
...
function https_request_api($apiPath, $data = null) {
    if(!$apiPath) {
        return false;
    }
...
//        curl_setopt($curl, curlopt_b, FALSE);
        ...
        curl_close($curl);
        return array_convert_encoding($output, "gbk" ,"utf-8");
    }else{
        return false;
    }
}

function array_convert_encoding($arr,$to_encoding= 'utf-8',$from_encoding = 'gbk')
{
    if(!is_array($arr))return $arr;
    foreach ($arr as $key => $value) {
        if (is_array($value)) {
           $arr[$key] = array_convert_encoding($value,$to_encoding,$from_encoding);
        }else{
