...
elseif($act=='apply'){
	...
	$joinsql="LEFT JOIN ".table('resume')." AS r ON f.resume_id=r.id";
    $heresql=" WHERE f.company_uid='{$_SESSION['uid']}'";

    $sql="SELECT fullname,telephone FROM ".table('resume');
    $result = $db->getall($sql);
    //if(($_POST['fullnme']==$result['fullname'] and $_POST['phone']==$result['telephone']))
     $smarty->assign('resume_list',get_my_apply($offset, $perpage,$joinsql.$wheresql));

   
   

    $smarty->display('member_company/company_apply.htm');
}
