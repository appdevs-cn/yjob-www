...
function tpl_function_qishi_jobs_show($params, &$smarty)
{
	...
	$aset=array_map("get_smarty_request",$aset);
        $infoRst = https_request_api("job/info/".$aset['id']);
        if($infoRst['codes'] == 'N') {
...
//	$sql = "select id,subsite_id,uid,audit,display,setmeal_deadline,add_mode,amount,company_id,district_cn,contents,refreshtime,jobspecial_cn,tag_cn,category,subclass,sdistrict,jobs_name,companyname,wage_cn,nature_cn,category_cn,sex_cn,age,education_cn,experience_cn,deadline,graduate,work_start,work_end,category_cns from ".table('jobs').$wheresql." LIMIT 1";
//	$val=$db->getone($sql);
//	if(empty($val))
//	{
//		$sql_tmp = "select id,subsite_id,uid,audit,display,setmeal_deadline,add_mode,amount,company_id,district_cn,contents,refreshtime,jobspecial_cn,tag_cn,category,subclass,sdistrict,jobs_name,companyname,wage_cn,nature_cn,category_cn,sex_cn,age,education_cn,deadline,experience_cn,work_start,work_end,category_cns from ".table('jobs_tmp').$wheresql." LIMIT 1";
...
//		exit();
//	}
	$shumu=$db->getone("select count(did) from ".table('personal_jobs_apply')." where jobs_id='{$val['id']}'");
	$category_cn=explode('/',$val['category_cn']);
	$count=count($category_cn);
	$ke1=$count-1;
	$val['category_cn']=$category_cn[$ke1];
	$val['counts']=$shumu['count(did)'];
	check_url($val['subsite_id'],$smarty,$_CFG['job_url']);
	$val['start_riqi']=date("n.j",$val['work_start']);
	$val['start_shijian']=date("G:i",$val['work_start']);
	$val['end_riqi']=date("n.j",$val['work_end']);
	$val['end_shijian']=date("G:i",$val['work_end']);
	...
	if ($val['jobspecial_cn'])
	{
		$jobspecial_cn=explode(',',$val['jobspecial_cn']);
		$val['jobspecial_cn']=$jobspecial_cn;
	}
	else
	{
	$val['jobspecial_cn']=array();
	}
	/*
		薪酬统计
