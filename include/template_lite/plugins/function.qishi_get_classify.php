...
if ($act=="QS_qgdistrict")
{
	$id=intval($aset['id']);
	$result = $db->query("SELECT * FROM ".table('category_qgdistrict')." where parentid=".$id." ORDER BY category_order desc,id asc".$limit);
	while($row = $db->fetch_array($result))
	{
	$row['categoryname']=cut_str($row['categoryname'],$aset['titlelen'],0,$aset['dot']);
	$row['title']=cut_str($row['categoryname'],$aset['titlelen'],0,$aset['dot']);
...
elseif ($act=="QS_qgdistrict_floor")
{
	$id=trim($aset['id']);
	$id=str_replace("_", ",", $id);
	$result = $db->query("SELECT * FROM ".table('category_qgdistrict')." where parentid in (".$id.") ORDER BY category_order desc,id asc".$limit);
	while($row = $db->fetch_array($result))
	{
	$row['categoryname']=cut_str($row['categoryname'],$aset['titlelen'],0,$aset['dot']);
	$row['title']=cut_str($row['categoryname'],$aset['titlelen'],0,$aset['dot']);
...
elseif ($act=="QS_qgdistrict_parent")
{
	if (strpos($aset['id'],"-"))
	{
		$arr=explode("-",$aset['id']);
	...
	{
		$result = $db->query("SELECT parentid FROM ".table('category_qgdistrict')." where id IN (".$idstr.")".$limit);
		while($row = $db->fetch_array($result))
		{
		$list[] =$row['parentid'];
		}
...
elseif ($act=="QS_shop_category")
{
	...
	$sql = "select * from ".table('shop_category')." ".$wheresql."  ORDER BY category_order desc,id asc".$limit;
	$result = $db->query($sql);
	while($row = $db->fetch_array($result))
	{
	$row['categoryname']=cut_str($row['categoryname'],$aset['titlelen'],0,$aset['dot']);
...
elseif ($act=="QS_shop_category_parent")
{
	if (strpos($aset['id'],"-"))
	{
		$arr=explode("-",$aset['id']);
	...
	{
		$result = $db->query("SELECT parentid FROM ".table('shop_category')." where id IN (".$idstr.")".$limit);
		while($row = $db->fetch_array($result))
		{
		$list[] =$row['parentid'];
		}
