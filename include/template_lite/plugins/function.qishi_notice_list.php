...
$result = $db->query("SELECT id,subsite_id,title,tit_color,tit_b,is_url,content,addtime,click FROM ".table('notice')." ".$wheresql.$orderbysql.$limit);
...
$row['url'] =$row['is_url']<>"http://"?$row['is_url']:url_rewrite($aset['showname'],array('id'=>$row['id']),1,$row['subsite_id']);
