...
if (isset($aset['page']))
{
	...
	$result = $db->query("SELECT id,subsite_id,jobname,detailed,refreshtime,comname,is_hot,amount,deadline,district_cn,sdistrict_cn FROM ".table('simple')." ".$wheresql.$orderbysql.$limit);
	$list   = array();
	//echo "SELECT * FROM ".table('jobs')." ".$wheresql.$orderbysql.$limit;
		...
		$row['simple_url']=url_rewrite($aset['simpleshow'],array('id'=>$row['id']),1,$row['subsite_id']);
		$list[] = $row;
		}
