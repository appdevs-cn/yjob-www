...
if (!empty($aset['citycategory']))
{
		...
		foreach($arr as $sid)
		{
				...
				{
				$xsql.= " OR sdistrict =".intval($cat[1])." OR  ( district =".intval($cat[0])." AND  sdistrict =0 )" ;
				}
...
if (!empty($aset['jobcategory']))
{
	...
	foreach($arr as $sid)
	{
		...
		{
		$xsql.= " OR subclass =".intval($cat[2])." OR  ( category =".intval($cat[1])." AND  subclass =0 )" ;
		}
	}
	$wheresql.=" AND  (".ltrim(ltrim($dsql.$xsql),'OR').") ";
}
...
if (!empty($aset['qgdistrict']))
{
	$dsql=$xsql="";
	$arr=explode("_",$aset['qgdistrict']);
	$arr=array_unique($arr);
	if (count($arr)>10) exit();
	foreach($arr as $sid)
	{
		...
		if (intval($cat[2])===0)
		{
		$dsql.= " OR categorys =".intval($cat[1]);
		}
		else
		{
		$xsql.= " OR subclasss =".intval($cat[2])." OR  ( categorys =".intval($cat[1])." AND  subclasss =0 )" ;
		}
	}
	$wheresql.=" AND  (".ltrim(ltrim($dsql.$xsql),'OR').") ";
}
...
{
			if (isset($aset['categorys'])  && $aset['categorys']<>'')
			{
				if (strpos($aset['categorys'],"-"))
				{
					$or=$orsql="";
					$arr=explode("-",$aset['categorys']);
					$arr=array_unique($arr);
					if (count($arr)>10) exit();
					$sqlin=implode(",",$arr);
					if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
					{
					$wheresql.=" AND topclasss IN  ({$sqlin}) ";
					}
				}
				else
				{
					$wheresql.=" AND topclasss = ".intval($aset['categorys']);
				}
			}
			if (isset($aset['subclasss'])  && $aset['subclasss']<>'')
			{
				if (strpos($aset['subclasss'],"-"))
				{
					$or=$orsql="";
					$arr=explode("-",$aset['subclasss']);
					$arr=array_unique($arr);
					if (count($arr)>10) exit();
					$sqlin=implode(",",$arr);
					if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
					{
						$wheresql.=" AND categorys IN  ({$sqlin}) ";
					}
				}
				else
				{
					$wheresql.=" AND categorys = ".intval($aset['subclasss']);
				}
			}
}
...
if (isset($aset['page']))
{
	...
	if (!empty($id))
	{
		...
		$result = $db->query("SELECT id,subsite_id,jobs_name,recommend,emergency,stick,highlight,companyname,company_id,company_audit,nature_cn,sex_cn,age,amount,category_cn,graduate,trade_cn,scale,scale_cn,district_cn,street_cn,jobspecial_cn,tag_cn,education_cn,experience_cn,wage,wage_cn,contents,setmeal_id,setmeal_name,refreshtime,click FROM ".table('jobs')." AS r ".$joinsql.$wheresql.$orderbysql);
		while($row = $db->fetch_array($result))
		{
			...
			if ($row['jobspecial_cn'])
			{
				$jobspecial_cn=explode(',',$row['jobspecial_cn']);
				$row['jobspecial_cn']=$jobspecial_cn;
			}
			else
			{
				$row['jobspecial_cn']=array();
			}
			//合并公司 显示模式
			if($aset['mode']==1)
			{
