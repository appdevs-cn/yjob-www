...
function tpl_function_qishi_hunter_show($params, &$smarty)
{
	...
	$sql = "select id,uid,huntername,companyname,contents,goodtrade_cn,goodcategory_cn,worktime_start,photo_img,cooperate_company,district_cn from ".table('hunter_profile').$wheresql." LIMIT 1";
	$row=$db->getone($sql);
	if (empty($row))
	{
