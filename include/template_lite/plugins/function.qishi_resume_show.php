...
if ($val)
{
	check_url($val['subsite_id'],$smarty,$_CFG['resume_url']);
	...
	if($apply){
		...
		$jobinfo = $db->getone("select subsite_id from ".table('jobs')." where id=".$apply['jobs_id']);
		if(empty($jobinfo)){
			$jobinfo = $db->getone("select subsite_id from ".table('jobs_tmp')." where id=".$apply['jobs_id']);
		}
		$val['jobs_url'] = url_rewrite('QS_jobsshow',array('id'=>$apply['jobs_id']),1,$jobinfo['subsite_id']);
	}
...
{
	global $db;
	$sql = "SELECT startyear,startmonth,endyear,endmonth,school,speciality,education_cn,todate FROM ".table('resume_education')." WHERE uid='".intval($uid)."' AND pid='".intval($pid)."'  ORDER BY startyear desc,startmonth desc ";
	return $db->getall($sql);
}
function get_this_work($uid,$pid)
{
	global $db;
	$sql = "select startyear,startmonth,endyear,endmonth,jobs,companyname,achievements,todate from ".table('resume_work')." where uid=".intval($uid)." AND pid='".$pid."' ORDER BY startyear desc,startmonth desc  " ;
	return $db->getall($sql);
}
function get_this_training($uid,$pid)
{
	global $db;
	$sql = "select startyear,startmonth,endyear,endmonth,agency,course,description,todate from ".table('resume_training')." where uid='".intval($uid)."' AND pid='".intval($pid)."' ORDER BY startyear desc,startmonth desc ";
	return $db->getall($sql);
}
