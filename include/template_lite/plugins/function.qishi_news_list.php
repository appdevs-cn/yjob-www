...
$result = $db->query("SELECT id,subsite_id,type_id,parentid,title,content,tit_color,tit_b,Small_img,author,source,focos,is_url,seo_description,seo_keywords,click,addtime FROM ".table('article')." ".$wheresql.$orderbysql.$limit);
$list= array();
while($row = $db->fetch_array($result))
{
	$wenzhang_type=$db->getone("select categoryname from ".table('article_property')." where id='{$row['focos']}'");
	$row['focos_categoryname']=$wenzhang_type["categoryname"];
	...
	{
		//var_dump($aset['showname'],array('id'=>$row['id']),1,$row['subsite_id']);
