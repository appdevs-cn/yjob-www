...
{
	global $db;
	$sql = "SELECT * FROM ".table('resume_education')." WHERE uid='".intval($uid)."' AND pid='".intval($id)."' ORDER BY startyear desc,startmonth desc ";
	return $db->getall($sql);
}
function get_this_work_all($uid,$id)
{
	global $db;
	$sql = "select * from ".table('resume_work')." where uid=".intval($uid)." AND pid='".$id."' ORDER BY startyear desc,startmonth desc " ;
	return $db->getall($sql);
}
function get_this_training_all($uid,$id)
{
	global $db;
	$sql = "select * from ".table('resume_training')." where uid='".intval($uid)."' AND pid='".intval($id)."' ORDER BY startyear desc,startmonth desc ";
	return $db->getall($sql);
}
...
function wap_url_rewrite($alias=NULL,$get=NULL,$rewrite=true,$subsite_id=0)
{
	...
	//var_dump($domain.$alias.".php".$url);exit();
...
function get_member_wap_url($type,$dirname=false,$uid=NULL)
{
	global $db,$_CFG;
	$type=intval($type);
	...
	{
		/* if($uid){
			$pidd=$db->getone("select * from ".table('resume')." where uid='{$uid}'");
			$idd=intval($pidd['id']);
			$return="personal/user.php?act=resume_basic&pid='{$idd}'";
		}else{
...
function wap_user_login($account,$password,$account_type=1,$uc_login=true,$expire=NULL,$uid=NULL)
{
	...
	if (!empty($usinfo))
	{
		...
		$login['qs_login']=get_member_wap_url($usinfo['utype'],false,$uid);
		$success=true;
		}
		else
		{
...
function user_login_new($account,$account_type=1,$uc_login=true,$expire=NULL)
{
	...
	if (!empty($usinfo))
	{
		wap_update_user_info($usinfo['uid'],true,true,1);
		return true;
	}
	return false;
}
...
function wap_update_user_info($uid,$record=true,$setcookie=true,$cookie_expire=NULL)
 {
	...
	if ($setcookie)
	{
		//$expire=intval($cookie_expire)>0?time()+30*3600*24*$cookie_expire:0;
		$expire=time()+30*3600*24;
			/* setcookie('QS[uid]',$user['uid'],$expire);
			setcookie('QS[username]',$user['username'],$expire);
			setcookie('QS[password]',$user['password'],$expire);
			setcookie('QS[utype]',$user['utype'], $expire);  */
...
function wap_add_resume_jobs_xin($pid,$uid,$str)
{
	global $db;
	$db->query("Delete from ".table('resume_jobs')." WHERE pid='".intval($pid)."'");
	$str=trim($str);
	...
	$setsqlarr['uid']=intval($uid);
	$setsqlarr['pid']=intval($pid);
	$setsqlarr['topclass']=intval($arr[0]);
	$setsqlarr['category']=intval($arr[1]);
	$setsqlarr['subclass']=intval($arr[2]);
	if (!$db->inserttable(table('resume_jobs'),$setsqlarr))return false;
...
function get_resume_basic_by_uids($id)
{
	global $db;
	$id=intval($id);
