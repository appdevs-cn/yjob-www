...
function check_resume($uid,$pid)
{
	...
	$resume_height=$resume_basic['height'];
	$resume_householdaddress=$resume_basic['householdaddress'];
	$resume_intention_jobs=$resume_basic['intention_jobs'];
	$resume_trade_cn=$resume_basic['trade_cn'];
	$resume_email=$resume_basic['email'];
	...
	if (!empty($resume_intention_jobs))$percent=$percent+10;
	if (!empty($resume_trade_cn))$percent=$percent+10;
	if (!empty($resume_height))$percent=$percent+5;
	if (!empty($resume_householdaddress))$percent=$percent+5;
	if (!empty($resume_email))$percent=$percent+10;
	...
	if (!empty($resume_photo))$percent=$percent+20;
...
function get_apply_jobs($offset,$perpage,$joinsql,$wheresql)
{
	...
	while($row = $db->fetch_array($result))
	{
		...
		$resume = $db->getone("select title from ".table('resume')." where id=".$row['resume_id']);
		$zhuangtai=$db->getone("select resume_state,resume_state_cn from ".table('company_label_resume')." where resume_id='{$row['resume_id']}' and jobs_id='{$row['jobs_id']}'");
		$row['zhuangtai']=$zhuangtai['resume_state'];
		$row['zhuangtai_cn']=$zhuangtai['resume_state_cn'];
		if($resume)
		{
			$row['resume_name'] = $resume['title'];
		}
		...
		else
		{
			...
			elseif($row['is_reply']=='5'){
				$row['reply_status'] = "已取消";
			}
			elseif($row['is_reply']=='6'){
			 	$row['reply_status'] = "已结束";
			}
			elseif($row['is_reply']=='7'){
				$row['reply_status'] = '已拒绝';
			}
		}
		$row_arr[] = $row;
	}
...
function count_interview($uid,$jobs_type,$look=NULL)
{
	...
	if (intval($look)>0) $wheresql.=" and i.jobs_id=j.id AND  i.personal_look=".intval($look);
	if($jobs_type==1)
	{
		$total_sql="SELECT COUNT(*) AS num FROM ".table('hunter_interview')." as i LEFT JOIN ".table('jobs')." AS j ON  i.jobs_id=j.id ".$wheresql;
	}
	else
	{
		$total_sql="SELECT COUNT(*) AS num FROM ".table('company_interview')." as i LEFT JOIN ".table('jobs')." AS j ON  i.jobs_id=j.id ".$wheresql;
	}
...
function count_personal_jobs_interviews($jobs_type,$uid,$look=NULL)
{
	global $db;
	$wheresql=" WHERE personal_uid='{$_SESSION['uid']}' ";
	if(intval($look)>0)	$wheresql.=" AND personal_interview='{$look}' ";
	if($jobs_type == 1)
	{
		$total_sql="SELECT COUNT(*) AS num FROM ".table('personal_hunter_jobs_apply').$wheresql;
	}
