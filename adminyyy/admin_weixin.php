...
{
	get_token();
	$uid = intval($_GET['uid'])==0?adminmsg("请选择会员！",1):intval($_GET['uid']);
...
{
	check_token();
	$setsqlarr['uid'] = intval($_POST['uid'])==0?adminmsg("请选择会员！",1):intval($_POST['uid']);
	$setsqlarr['utype'] = intval($_POST['utype']);
	$setsqlarr['username'] = trim($_POST['username']);
	$setsqlarr['weixin_openid'] = trim($_POST['weixin_openid'])==''?adminmsg("参数错误！",1):trim($_POST['weixin_openid']);
	$setsqlarr['content'] = trim($_POST['content'])==''?adminmsg("请输入消息内容！",1):trim($_POST['content']);
...
elseif($act == 'send_weixin_msgqueue_save')
{
	...
	$content = trim($_POST['content'])==''?adminmsg("请输入消息内容！",1):trim($_POST['content']);
	if($utype==0){
