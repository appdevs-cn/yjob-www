...
elseif($act == 'set_account_save')
{
	...
	$setsqlarr['mobile_audit']=intval($_POST['mobile_audit']);
        $setsqlarr['invite_code'] = intval($_POST['code']);
...
elseif($act == 'matching'){
	...
	if($resume_jobs_topclass=='' && $resume_jobs_category=='')
	{
		$link[0]['text'] = "返回简历列表";
		$link[0]['href'] = '?act=list';
		adminmsg('该简历未匹配到合适的职位！',1,$link);
	}
...
elseif($act == 'import')
{
	...
	{
		if(trim($data->sheets[0]['cells'][$i][2]) == '姓名' && trim($data->sheets[0]['cells'][$i][1]) == '简历名称'){
			continue;
		}
		...
		if($userid>0)
		{
			...
			$subsite_id  = trim($data->sheets[0]['cells'][$i][23])?trim($data->sheets[0]['cells'][$i][23]):'0';
			...
			if($pid)
			{
				...
				$searchtab['audit']=1;
				$searchtab['subsite_id'] = $subsite_id;
	...
	$joinsql=" left JOIN ".table('members')." as m ON o.uid=m.uid LEFT JOIN  ".table('members_info')." as c ON o.uid=c.uid ";
...
}elseif($act == 'order_set')
{
	...
	$smarty->display('personal/admin_order_set.htm');
...
}elseif($act == 'show_order')
{
	...
	$smarty->assign('url',$_SERVER["HTTP_REFERER"]);
	$smarty->assign('payment',get_order_one($_GET['id']));
	$smarty->display('personal/admin_order_show.htm');
...
} elseif($act == 'invite_code'){
    check_token();
    check_permissions($_SESSION['admin_purview'],"per_user_edit");
    if(set_user_invite_code(intval($_POST['code']),intval($_POST['invitecode_uid'])))
    {
            $link[0]['text'] = "返回列表";
            $link[0]['href'] = $_POST['url'];
            adminmsg('操作成功！',2,$link);
