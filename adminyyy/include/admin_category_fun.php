...
function get_category_qgdistrict()
{
	global $db;
	...
	while($row = $db->fetch_array($result))
	{		
		$sql = "select * from ".table('category_qgdistrict')." where parentid=".$row['id']."  order BY category_order desc,id asc";
		$sub=$db->getall($sql);
		$row['sub']=$sub;
		$category[]=$row;
	}
...
function get_category_qgdistrict_one($id)
{
	global $db;
	$sql = "select * from ".table('category_qgdistrict')." WHERE id=".intval($id)." LIMIT 1";
	return $db->getone($sql);
}
...
function del_qgdistrict_category($id)
{
	global $db;
	if(!is_array($id)) $id=array($id);
	$return=0;
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		if (!$db->query("Delete from ".table('category_qgdistrict')." WHERE id IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
		if (!$db->query("Delete from ".table('category_qgdistrict')." WHERE parentid IN (".$sqlin.") ")) return false;
		$return=$return+$db->affected_rows();
	}
	return $return;
}
