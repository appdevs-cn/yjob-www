...
function makejs_classify()
{
	...
	// unset($third_arr);
	$sql = "select * from ".table('category_jobs')." where parentid=1065 order BY category_order desc,id asc";
	$listds=$db->getall($sql);
	foreach($listds as $parentds)
	{
	$parentarrds[]="\"".$parentds['id'].",".$parentds['categoryname']."\"";
	}
	$content .= "var QS_jobs_ds=new Array(".implode(',',$parentarrds).");\n";	
	unset($parentarrds);
	//全国地区分类
	$sql = "select * from ".table('category_qgdistrict')." where parentid=0 order BY category_order desc,id asc";
	$list=$db->getall($sql);
	foreach($list as $parent)
	{
	$parentarr[]="\"".$parent['id'].",".$parent['categoryname']."\"";
	}
	$content .= "var QS_qgdistrict_parent=new Array(".implode(',',$parentarr).");\n";	
	unset($parentarr);
	$content .= "var QS_qgdistrict=new Array(); \n";
	$content_third = "";
	foreach($list as $val)
	{
		$sql1 = "select * from ".table('category_qgdistrict')." where parentid=".$val['id']."  order BY category_order desc,id asc";
		$list1=$db->getall($sql1);
		if (is_array($list1))
		{	
			foreach($list1 as $val1)
			{
				$sarr[]=$val1['id'].",".$val1['categoryname'];
				$sql2 = "select * from ".table('category_qgdistrict')." where parentid=".$val1['id']."  order BY category_order desc,id asc";
				$list2=$db->getall($sql2);
				if (is_array($list2))
				{	
					foreach($list2 as $val2)
				...
				$content_third .= "QS_qgdistrict[".$val1['id']."]=\"".implode('|',$third_arr)."\"; \n";
				unset($third_arr);
				}
			}
		$content .= "QS_qgdistrict[".$val['id']."]=\"".implode('|',$sarr)."\"; \n";	
		unset($sarr);
		}
	}
	// $content .= "var QS_jobs_third=new Array(); \n";
	$content .= $content_third;	
	//z职位特点
	$sqlqq = "select * from ".table('category')." where c_alias = 'jobspecial'";
	$listdss=$db->getall($sqlqq);
	foreach($listdss as $parentdss)
	{
	$parentarrdss[]="\"".$parentdss['c_id'].",".$parentdss['c_name']."\"";
	}
	$content .= "var jobspecial=new Array(".implode(',',$parentarrdss).");\n";	
	unset($parentarrdss);
...
function tag_link($content){
	 global $_CFG;
     $limit = $_CFG['content_key_link']; //一篇文章中同一个关键字最多出现多少次描文本（建议不超过2次）
