...
function get_resume_list($offset,$perpage,$get_sql= '')
{
	...
	while($row = $db->fetch_array($result))
	{
    $worknum=$db->get_total("select COUNT(*) AS num from  ".table("company_label_resume")." where resume_id=".$row['id']." and resume_state=3 ");
	$worknum1=$db->get_total("select COUNT(*) AS num from  ".table("company_label_resume")." where resume_id=".$row['id']." and resume_state=4 ");
	$row['worknum']=$worknum;
	$row['worknum1']=$worknum1;
	$row['resume_url']=url_rewrite('QS_resumeshow',array('id'=>$row['id'],'dsf'=>5),1,$row['subsite_id']);
	$row_arr[] = $row;
	}
	return $row_arr;
}
...
function set_user_invite_code($code, $uid) {
    global $db;
    if (!$db->query("UPDATE ".table('members')." SET invite_code= {$code} WHERE uid={$uid} LIMIT 1")) return false;
    return true;
}
//导入简历时的注册会员
