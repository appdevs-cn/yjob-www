...
function get_news($offset, $perpage, $sql= '')
{
	...
	$result = $db->query("SELECT a.*,c.id as cid,c.categoryname as c_categoryname,p.id as pid,p.categoryname as p_categoryname,a.subsite_id FROM ".table('article')." AS a ".$sql."  ".$limit);
	...
	{
	$row['url'] = url_rewrite('QS_newsshow',array('id'=>$row['id']),1,$row['subsite_id']);
	}
