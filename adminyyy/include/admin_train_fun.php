 ...
 function del_setmeal_one($id)
{
	...
	//填写管理员日志
	write_log("后台成功删除套餐", $_SESSION['admin_name'],3);
	return true;
}
 ...
 function del_train_img($id)
{
	...
	write_log("删除图片id为".$sqlin."的培训图片,共删除".$num."行", $_SESSION['admin_name'],3);
 ...
 function edit_img_audit($id,$audit,$reason,$pms_notice='1')
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("将图片id为".$sqlin."的培训图片的审核状态设置为".$audit.",共操作".$return."行", $_SESSION['admin_name'],3);
...
function edit_news_audit($id,$audit,$reason,$pms_notice='1')
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("将新闻id为".$sqlin."的培训新闻的审核状态设置为".$audit.",共操作".$return."行", $_SESSION['admin_name'],3);
		if($audit=='1') {$note='成功通过网站管理员审核!';}elseif($audit=='2'){$note='正在审核中!';}else{$note='未通过网站管理员审核！';}
	    $reason=$reason==''?'无':$reason;
		//发送站内信
		if ($pms_notice=='1')
...
function del_train_news($id)
{
	...
	$num=$db->affected_rows();
	write_log("删除新闻id为".$sqlin."的培训新闻,共删除".$num."行", $_SESSION['admin_name'],3);
	return $db->affected_rows();
}
...
function edit_train_audit($uid,$audit,$reason,$pms_notice)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("将会员uid为".$sqlin."的培训机构的审核状态设置为".$audit, $_SESSION['admin_name'],3);
		//发送站内信
		if ($pms_notice=='1')
		{
			$reason=$reason==''?'无':$reason;
...
function refresh_train($uid,$refrescou=false)
{
	...
	write_log("刷新会员uid为".$sqlin."的培训机构,共刷新".$return."行", $_SESSION['admin_name'],3);
	return $return;
}
function del_train($uid)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除会员uid为".$sqlin."的培训机构", $_SESSION['admin_name'],3);
	return true;
	}
	return false;
}
function del_train_allteacher($uid)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除会员uid为".$sqlin."的讲师", $_SESSION['admin_name'],3);
		return true;
	}
	return false;
}
function del_train_allcourse($uid)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除会员uid为".$sqlin."的课程", $_SESSION['admin_name'],3);
		return true;
	}
	return false;
}
...
function del_train_idteacher($id)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除讲师id为".$sqlin."的讲师", $_SESSION['admin_name'],3);
		return true;
	}
	return false;
}
...
function del_train_teacourse($teacher_id)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除讲师id为".$sqlin."的课程", $_SESSION['admin_name'],3);
		return true;
	}
	return false;
}
...
function del_meal_log($id)
{
	...
	$num=$db->affected_rows();
	write_log("删除套餐变更记录id为".$sqlin."的餐变更记录,共删除".$num."行", $_SESSION['admin_name'],3);
	return $db->affected_rows();
}
...
function del_course($id)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		write_log("删除课程id为".$sqlin."的课程,共删除".$return."行", $_SESSION['admin_name'],3);
		return $return;
	}
	else
	{
...
function edit_course_audit($id,$audit,$reason,$pms_notice='1')
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("将课程id为".$sqlin."的课程的审核状态设置为".$audit."共操作".$return."行", $_SESSION['admin_name'],3);
			//发送站内信
			if ($pms_notice=='1')
			{
...
function refresh_course($id)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("刷新课程id为".$sqlin."的课程,共刷新".$return."行", $_SESSION['admin_name'],3);
	}
	return $return;
}
function refresh_teacher($id)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("刷新讲师id为".$sqlin."的讲师,共操作".$return."行", $_SESSION['admin_name'],3);
	}
	return $return;
}
function recom_course($id,$recommend,$pms_notice,$notice)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("将课程id为".$sqlin."的课程的推广状态设置为".$recommend.",共操作".$return."行", $_SESSION['admin_name'],3);
		//发送站内信
		if ($pms_notice=='1')
		{
				$result = $db->query("SELECT course_name,uid FROM ".table('course')." WHERE id IN ({$sqlin})");
...
function recom_teacher($id,$recommend,$pms_notice,$notice)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("将讲师id为".$sqlin."的讲师,推广状态设为".$recommend, $_SESSION['admin_name'],3);
		//发送站内信
		if ($pms_notice=='1')
		{
...
function recom_train($id,$recommend,$pms_notice,$notice)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		$return=$return+$db->affected_rows();
		write_log("将会员uid为".$sqlin."的培训机构的推广状态设置为".$recommend.",共删除".$return."行", $_SESSION['admin_name'],3);
		//发送站内信
		if ($pms_notice=='1')
		{
				$result = $db->query("SELECT trainname,uid FROM ".table('train_profile')." WHERE uid IN ({$sqlin})");
...
function delay_course($id,$days)
{
	...
	if (preg_match("/^(\d{1,10},)*(\d{1,10})$/",$sqlin))
	{
		...
		while($row = $db->fetch_array($result))
		{
			...
			$return=$return+$db->affected_rows();
			write_log("将课程id为".$sqlin."的课程，延期,共操作".$return."行", $_SESSION['admin_name'],3);
		}
	}
	return $return;
