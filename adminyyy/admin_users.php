...
elseif($act == 'users_set')
{
	...
	$smarty->assign('admin_site_set',explode(',',$account['site_purview']));
...
elseif($act == 'users_set_sitepurview_save')
{
	check_token();
	$id=intval($_POST['id']);
	if ($_SESSION['admin_purview']<>"all")adminmsg("权限不足！",1);
	$setsqlarr['site_purview']=$_POST['site_purview'];
	$setsqlarr['site_purview']=implode(',',$setsqlarr['site_purview']);
		if ($db->updatetable(table('admin'),$setsqlarr,' admin_id='.$id))
		{
			//填写管理员日志
			write_log("成功设置分站管理权限", $_SESSION['admin_name'],3);
			adminmsg("设置成功！",2);
		 }
		 else
		{
			//填写管理员日志
			write_log("设置分站管理权限失败", $_SESSION['admin_name'],3);
			adminmsg("设置失败！",0);
		 }
}
?>