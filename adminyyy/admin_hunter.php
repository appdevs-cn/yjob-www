...
elseif ($act=='editjobs_save')
{
	...
	if($utype=='3'){
		$setsqlarr['key']=$setsqlarr['jobs_name'].$setsqlarr['companyname'].$setsqlarr['category_cn'].$setsqlarr['contents'].$setsqlarr['jobs_qualified'];
		require_once(QISHI_ROOT_PATH.'include/splitword.class.php');
		$sp = new SPWord();
		$setsqlarr['key']="{$setsqlarr['jobs_name']} {$setsqlarr['companyname']} ".$sp->extracttag($setsqlarr['key']);
		$setsqlarr['key']=$sp->pad($setsqlarr['key']);
		$setsqlarr['likekey']=$setsqlarr['jobs_name'].','.$setsqlarr['companyname'].','.$setsqlarr['category_cn'];
	}elseif($utype=='1'){
		$setsqlarr['key']=$setsqlarr['jobs_name'].$companyname1.$setsqlarr['category_cn'].$setsqlarr['contents'].$setsqlarr['jobs_qualified'];
		require_once(QISHI_ROOT_PATH.'include/splitword.class.php');
		$sp = new SPWord();
		$setsqlarr['key']="{$setsqlarr['jobs_name']} {$companyname1} ".$sp->extracttag($setsqlarr['key']);
		$setsqlarr['key']=$sp->pad($setsqlarr['key']);
		$setsqlarr['likekey']=$setsqlarr['jobs_name'].','.$companyname1.','.$setsqlarr['category_cn'];
	}
	if ($days>0)
	{
	...
	$wheresql=" id='".$id."' ";
	if (!$db->updatetable(table('hunter_jobs'),$setsqlarr,$wheresql)) adminmsg("保存失败！",0);
...
elseif ($act=='hunter_profile_save')
{
	...
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
	if ($db->updatetable(table('hunter_profile'),$setsqlarr,$wheresql))
	{
		write_log("修改猎头id为".$id."的猎头信息", $_SESSION['admin_name'],3);
		unset($setsqlarr);
...
elseif($act == 'edit_setmeal_save')
{
	...
	if ($hunter_uid)
	{
			...
			if (!$db->updatetable(table('members_hunter_setmeal'),$setsqlarr," uid=".$hunter_uid."")) adminmsg('修改出错！',0);
 		//会员套餐变更记录。管理员后台修改会员套餐：修改会员。3表示：管理员后台修改
 			...
 			if ($setsqlarr['endtime']<>"")
			{
				...
				if (!$db->updatetable(table('hunter_jobs'),$setmeal_deadline," uid='{$hunter_uid}' AND add_mode='2' "))adminmsg('修改出错！',0);
			}
	}
	write_log("修改会员uid为".intval($_POST['hunter_uid'])."的套餐", $_SESSION['admin_name'],3);
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
	adminmsg('操作成功！',2,$link);
}
