...
elseif ($act=='editteacher_save'){
	...
	if (!$db->updatetable(table('train_teachers'),$setsqlarr,$wheresql)) adminmsg("保存失败！",0);
	$teacherarr['teacher_cn']=$setsqlarr['teachername'];
	$wheresql=" teacher_id=".$setsqlarr['id'];
	if (!$db->updatetable(table('course'),$teacherarr,$wheresql)) adminmsg("保存失败！",0);
	write_log("修改讲师id为".$setsqlarr['id']."的讲师信息", $_SESSION['admin_name'],3);
	$link[0]['text'] = "返回讲师列表";
...
elseif ($act=='editcourse_save')
{
	...
	$wheresql=" id='".$id."' ";
	if (!$db->updatetable(table('course'),$setsqlarr,$wheresql)) adminmsg("保存失败！",0);
	$wheresql=" pid=".$id;
	if (!$db->updatetable(table('course_contact'),$setsqlarr_contact,$wheresql)) adminmsg("保存失败！",0);
	write_log("修改课程id为".$id."的课程信息", $_SESSION['admin_name'],3);
	$link[0]['text'] = "返回课程列表";
	$link[0]['href'] = $_POST['url'];
	adminmsg("修改成功！",2,$link);
}
...
elseif ($act=='train_profile_save')
{
	...
	$link[0]['href'] = $_POST['url'];
		if ($db->updatetable(table('train_profile'),$setsqlarr,$wheresql))
		{
				...
				if (!$db->updatetable(table('course'),$coursearr," uid=".$setsqlarr['uid']."")) adminmsg('修改培训机构名称出错！',0);
				if (!$db->updatetable(table('train_teachers'),$coursearr," uid=".$setsqlarr['uid']."")) adminmsg('修改培训机构名称出错！',0);
				write_log("修改会员uid为".$setsqlarr['uid']."的培训机构信息", $_SESSION['admin_name'],3);
				unset($setsqlarr);
...
elseif($act == 'userpass_edit')
{
	...
	write_log("修改会员uid为".$user_info['uid']."的密码", $_SESSION['admin_name'],3);
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
...
elseif($act == 'edit_setmeal_save')
{
	...
	if ($train_uid)
	{
			...
			if (!$db->updatetable(table('members_train_setmeal'),$setsqlarr," uid=".$train_uid."")) adminmsg('修改出错！',0);
		...
		//会员套餐变更记录。管理员后台修改会员套餐：修改会员。3表示：管理员后台修改
			...
			if ($setsqlarr['endtime']<>"")
			{
				...
				if (!$db->updatetable(table('course'),$setmeal_deadline," uid='{$train_uid}' AND add_mode='2' "))adminmsg('修改出错！',0);
			}
	}
	write_log("修改会员uid为".$train_uid."套餐", $_SESSION['admin_name'],3);
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = $_POST['url'];
	adminmsg('操作成功！',2,$link);
}
elseif($act == 'set_setmeal_save')
{
    ...
    if (intval($_POST['reg_service'])>0)
	{
		...
		write_log("编辑会员uid为".$_POST['train_uid']."套餐信息", $_SESSION['admin_name'],3);
		adminmsg('操作成功！',2,$link);
		}
		else
		{
...
elseif($act == 'train_news_save')
{
	...
	!$db->updatetable(table('train_news'),$setsqlarr,' id='.$id.' ')?adminmsg("修改机构新闻失败！",1,$link):adminmsg("修改机构新闻成功！",2,$link);
}
