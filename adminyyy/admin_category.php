...
elseif($act == 'qgdistrict')
{
	get_token();
	$smarty->assign('navlabel',"qgdistrict");
	$smarty->assign('district',get_category_qgdistrict());
	$smarty->display('category/admin_category_qgdistrict.htm');
}
elseif($act == 'qgdistrict_all_save')
{
	check_token();
	if (is_array($_POST['save_id']) && count($_POST['save_id'])>0)
	{
		for ($i =0; $i <count($_POST['save_id']);$i++){
			if (!empty($_POST['categoryname'][$i]))
			{
				...
				!$db->updatetable(table('category_qgdistrict'),$setsqlarr," id=".intval($_POST['save_id'][$i]))?adminmsg("保存失败！",0):"";
				$num=$num+$db->affected_rows();
			}
		}
		//填写管理员日志
		write_log("后台成功更新全国地区分类！本次更新了{$num}个分类", $_SESSION['admin_name'],3);
	}
	//新增的入库
	if (is_array($_POST['add_pid']) && count($_POST['add_pid'])>0)
	{
		for ($i =0; $i <count($_POST['add_pid']);$i++){
			if (!empty($_POST['add_categoryname'][$i]))
			{
				...
				$setsqlarr['content']=trim($_POST['content'][$i]);
				$setsqlarr['parentid']=intval($_POST['add_pid'][$i]);
				!$db->inserttable(table('category_qgdistrict'),$setsqlarr)?adminmsg("保存失败！",0):"";
				$num=$num+$db->affected_rows();
			}

		}
		//填写管理员日志
		write_log("后台成功添加全国地区分类！本次添加了{$num}个分类", $_SESSION['admin_name'],3);
	}
	makejs_classify();
	makejs_train_classify();
	adminmsg("保存成功！",2);
}
elseif($act == 'del_qgdistrict_category')
{
	check_token();
	$id=$_REQUEST['id'];
	if ($num=del_qgdistrict_category($id))
	{
		//填写管理员日志
		write_log("后台成功删除全国地区分类！共删除".$num."行", $_SESSION['admin_name'],3);
		adminmsg("删除成功！共删除".$num."行",2);
	}
	else
	{
...
elseif($act == 'edit_qgdistrict_category')
{
	get_token();
	$smarty->assign('navlabel',"qgdistrict");
	$smarty->assign('category',get_category_qgdistrict_one($_GET['id']));
	$smarty->display('category/admin_category_qgdistrict_edit.htm');
}
elseif($act == 'edit_qgdistrict_category_save')
{
	check_token();
	$setsqlarr['categoryname']=!empty($_POST['categoryname']) ?trim($_POST['categoryname']) : adminmsg("请填写名称",1);
	$setsqlarr['category_order']=intval($_POST['category_order']);
	...
	$setsqlarr['parentid']=intval($_POST['parentid']);
	!$db->updatetable(table('category_qgdistrict'),$setsqlarr," id=".intval($_POST['id']))?adminmsg("修改失败！",0):"";
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = '?act=qgdistrict';
	makejs_classify();
	makejs_train_classify();
	//填写管理员日志
	write_log("后台成功修改全国地区分类！", $_SESSION['admin_name'],3);
	adminmsg("保存成功！",2,$link);
}
elseif($act == 'add_category_qgdistrict')
{
	get_token();
	$smarty->assign('navlabel',"qgdistrict");
	$smarty->display('category/admin_category_qgdistrict_add.htm');
}
elseif($act == 'add_category_qgdistrict_save')
{
	check_token();
	//新增的入库
	if (is_array($_POST['categoryname']) && count($_POST['categoryname'])>0)
	...
	if (is_array($_POST['save_id']) && count($_POST['save_id'])>0)
	{
		for ($i =0; $i <count($_POST['save_id']);$i++){
			if (!empty($_POST['categoryname'][$i]))
			{
				$setsqlarr['categoryname']=trim($_POST['categoryname'][$i]);
				$setsqlarr['category_order']=intval($_POST['category_order'][$i]);
				!$db->updatetable(table('category_hunterjobs'),$setsqlarr," id=".intval($_POST['save_id'][$i]))?adminmsg("保存失败！",0):"";
				$num=$num+$db->affected_rows();
			}
		}
		//填写管理员日志
		write_log("后台成功更新猎头职位分类！本次更新了".$num."个分类", $_SESSION['admin_name'],3);
	}
	//新增的入库
	if (is_array($_POST['add_pid']) && count($_POST['add_pid'])>0)
	{
...
elseif($act == 'del_hunterjobs_category')
{
	...
	{
		//填写管理员日志
		write_log("后台成功删除猎头职位分类！本次删除了".$num."个分类", $_SESSION['admin_name'],3);
		adminmsg("删除成功！共删除".$num."行",2);
	}
	else
	{
...
elseif($act == 'edit_hunterjobs_category_save')
{
	...
	$setsqlarr['category_order']=intval($_POST['category_order']);
	$setsqlarr['parentid']=intval($_POST['parentid']);
	!$db->updatetable(table('category_hunterjobs'),$setsqlarr," id=".intval($_POST['id']))?adminmsg("修改失败！",0):"";
	$link[0]['text'] = "返回列表";
	$link[0]['href'] = '?act=hunter_jobs';
	makejs_classify();
	makejs_train_classify();
	//填写管理员日志
	write_log("后台成功修改猎头职位分类！", $_SESSION['admin_name'],3);
	adminmsg("保存成功！",2,$link);
}
