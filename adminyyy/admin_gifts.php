...
elseif($act == 'generate_save')
{
	...
	write_log("生成礼品卡，共生成 {$number} 行", $_SESSION['admin_name'],3);
...
elseif($act == 'gifts_act')
{
	...
	if ($_POST['deleteid'])
	{
		...
		if ($num=del_gifts($id))
		{
		write_log("删除礼品卡,共删除".$num."行", $_SESSION['admin_name'],3);
		adminmsg("删除成功！共删除".$num."行",2);
		}
		else
		{
...
elseif($act == 'add_category_save')
{
	...
	!$db->inserttable(table('gifts_type'),$setsqlarr)?adminmsg("添加失败！",0):"";
	$link[0]['text'] = "返回分类管理";
	$link[0]['href'] = '?act=category';
	$link[1]['text'] = "继续添加";
...
elseif($act == 'edit_category_save')
{
	...
	!$db->updatetable(table('gifts_type'),$setsqlarr," t_id=".$id."")?adminmsg("修改失败！",0):adminmsg("修改成功！",2,$link);
}
elseif($act == 'del_category')
{
	...
	if ($num>0)
	{
	write_log("删除礼品卡分类,共删除".$num."行", $_SESSION['admin_name'],3);
	adminmsg("删除成功！共删除".$num."行",2);
	}
	else
	{
