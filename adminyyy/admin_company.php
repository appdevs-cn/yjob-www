...
elseif ($act=='company_profile_save')
{
	...
	$link[0]['href'] = $_POST['url'];
		if ($db->updatetable(table('company_profile'),$setsqlarr,$wheresql))
		{
				...
				if (!$db->updatetable(table('jobfair_exhibitors'),array('companyname'=>$jobarr['companyname'])," uid=".intval($_POST['cuid'])."")) adminmsg('修改公司名称出错！',0);
				...
				write_log("修改企业uid为".intval($_POST['cuid'])."的企业资料", $_SESSION['admin_name'],3);
...
elseif($act == 'company_news_save')
{
	...
	!$db->updatetable(table('company_news'),$setsqlarr,' id='.$id.' ')?adminmsg("修改企业新闻失败！",1,$link):adminmsg("修改企业新闻成功！",2,$link);
