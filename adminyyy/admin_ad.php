...
if($act == 'list')
{
	...
	{
		$wheresql.=empty($wheresql)?" WHERE ":" AND ";
		$wheresql.=" (a.subsite_id=0 or a.subsite_id=".intval($_GET['subsite_id']).") ";
	}
	...
	$smarty->assign('ad_category',get_ad_category(NULL,intval($_GET['subsite_id'])));
...
elseif($act == 'edit_ad')
{
	...
	$ad=get_ad_one($id);
	$smarty->assign('ad',$ad);
	$smarty->assign('ad_category',get_ad_category(NULL,intval($ad['subsite_id'])));//广告位分类列表
...
elseif($act=='ad_category_add_save')
{
	...
	$setsqlarr['tpl']=$_POST['tpl']?implode(',',$_POST['tpl']):adminmsg('您没有选择所属模板！',1);
...
elseif($act=='edit_ad_category')
{
	...
	$ad_category['tpl'] = explode(',',$ad_category['tpl']);
...
elseif($act == 'order_notes_save')
{
	...
	//填写管理员日志
	write_log("后台修改广告订单", $_SESSION['admin_name'],3);
